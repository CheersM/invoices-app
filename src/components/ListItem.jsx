...
function ListItem({ items, onRemove, onModal, onEditInvoice }) {
  const removeInvoice = (item) => {
    if (window.confirm('Are you sure you want to delete the invoice?')) {
      axios.delete('http://localhost:3001/invoices/' + item.id).then(() => {
        onRemove(item.id);
      });
    }
  };

  const editInvoice = (invoice, id) => {
    onModal(true);
    const newValue = onEditInvoice(invoice);
    if (!newValue) {
      return;
    }

    const newList = items.map((invoice) => {
      if (invoice.id === id) {
        invoice.number = newValue;
        invoice.date_created = newValue;
        invoice.date_supplied = newValue;
        invoice.comment = newValue;
      }
    });
    if (newList) {
      onEditInvoice(invoice, newList);
      axios
        .patch('http://localhost:3001/invoices/' + invoice.id, {
          number: newList,
          date_created: newList,
          date_supplied: newList,
          comment: newList,
        })
        .catch(() => {
          alert('Failed to change invoice');
        });
    }
  };
  ...
  return (
    <Fragment>
      {items &&
        items.map((obj, index) => (
          <div key={index} className="invoice_block">
            <p>{obj.date_created}</p>
            <p>{obj.number}</p>
            <p>{obj.date_supplied}</p>
            <p>{obj.comment}</p>
            <div className="invoice-opt">
              <svg
                onClick={() => editInvoice(obj)}
                width="18"
                height="18"
                viewBox="0 0 15 15"
                fill="none"
